...
## 37. Quarto bug real no Windows: `npm.ps1` bloqueado pela mesma razão dos scripts do projeto

Com a seção 36 corrigida, o mesmo usuário avançou até a preparação do frontend, onde `npm install` falhou: `"O arquivo ...\npm.ps1 não pode ser carregado. O arquivo ...\npm.ps1 não está assinado digitalmente."` — o mesmo tipo de bloqueio de execução de scripts baixados da internet já visto e resolvido (via `Unblock-File`) para `iniciar-sem-docker.ps1`/`parar-sem-docker.ps1` (seção 34), mas desta vez no próprio `npm.ps1` que vem dentro da distribuição `.zip` portátil do Node.js — instalada sem administrador, exatamente o cenário documentado em `PRIMEIRO_USO.md`. O PowerShell, ao digitar `npm`, prioriza `npm.ps1` sobre `npm.cmd` quando ambos existem na mesma pasta; como o `.zip` inteiro foi extraído de um arquivo baixado pelo navegador, todo o seu conteúdo herda a marca de "zona da internet", incluindo os wrappers `.ps1`.

**Correção**: `iniciar-sem-docker.ps1` passou a desbloquear automaticamente todos os `.ps1` da pasta onde o `npm` resolvido está instalado (via `Get-Command npm` → `Split-Path` do diretório → `Get-ChildItem -Filter "*.ps1" | Unblock-File`), logo antes de rodar `npm install` — sem exigir que o usuário descubra e rode esse comando manualmente. Só se aplica quando necessário (`Unblock-File` em um arquivo já desbloqueado é inofensivo) e não depende de saber previamente onde o Node.js foi instalado, já que usa o mesmo `npm` que o restante do script já usa.

Quarto bug seguido — e o quarto encontrado só por teste real, nunca por revisão de código — todos nesta mesma sessão de validação com o usuário. Cada instalação sem administrador (scripts do projeto, depois o próprio Node.js) precisou do mesmo tipo de desbloqueio, então a correção desta seção elimina a categoria inteira do problema para o `npm`, não só a ocorrência pontual.

## 38. Quinto bug real no Windows: caminho com espaço quebrava o comando do frontend

Depois da seção 37, o mesmo usuário chegou a rodar `npm install` com sucesso, mas o frontend falhou com `Error: Cannot find module 'C:\Users\kairu\Downloads\BI-Demanda-main'` — um caminho visivelmente cortado (faltando o restante: `\frontend\node_modules\vite\bin\vite.js`). A pasta onde o projeto estava extraído desta vez se chamava `BI-Demanda-main (1)` (o Windows adiciona esse sufixo automaticamente ao extrair um ZIP baixado pela segunda vez sem apagar o anterior) — o **espaço antes do `(1)`** é a causa: `Start-Process -ArgumentList` no PowerShell junta os elementos do array com espaço para montar a linha de comando final, mas **não coloca aspas automaticamente** em elementos que já contêm espaço. O caminho completo do `vite.js`, passado como um elemento do array, foi quebrado em dois argumentos separados na hora de montar o comando — o `node` recebeu `C:\Users\kairu\Downloads\BI-Demanda-main` como o módulo principal (primeiro token antes do espaço) e tentou carregá-lo como se fosse o arquivo de entrada, falhando exatamente como reproduzido.

Só afetava o frontend porque é o único `-ArgumentList` do script que inclui um caminho completo baseado em `$RootDir` (o backend passa `"app.main:app"`, uma string fixa sem caminho, e por isso nunca teria o mesmo problema, mesmo com espaço na pasta).

**Correção**: o elemento do array que contém o caminho do `vite.js` passou a incluir suas próprias aspas duplas (`"`"$viteJs`""`), garantindo que `Start-Process` sempre trate o caminho inteiro como um único argumento na linha de comando, com ou sem espaço no nome da pasta — não depende mais de o usuário nunca ter um espaço em lugar nenhum do caminho (algo fora do controle do script, já que nomes como `(1)`, `Nova pasta` ou `OneDrive - Empresa` são comuns no Windows).

Quinto bug seguido encontrado só por teste real — nenhum destes cinco (seções 34-38) teria aparecido numa revisão de código sem alguém genuinamente rodando os scripts em uma máquina Windows real, muitas vezes numa combinação específica de estado (uma segunda cópia da pasta, um Node extraído sem admin, um usuário sem Git) que é exatamente o público-alvo desta preparação.

## 39. Sexto bug real no Windows: `.venv` quebrado depois de renomear a pasta do projeto

Como workaround da seção 38 (caminho com espaço quebrando `Start-Process -ArgumentList`), o usuário renomeou a pasta do projeto de `BI-Demanda-main (1)` para `BI-Demanda-main2` — mas essa pasta já tinha um `backend\.venv` criado numa execução anterior, dentro do caminho antigo. Depois de renomear, `.\iniciar-sem-docker.ps1` voltou a travar em "O backend demorou demais para responder", mas desta vez **sem nenhuma linha de log** — nem sucesso, nem erro. Isso descartava tanto o bug da seção 36 (o log mostraria o Uvicorn subindo) quanto qualquer problema de porta ocupada (o `parar-sem-docker.ps1` da execução anterior já tinha confirmado o encerramento antes da renomeação).

Causa: no Windows, o `venv` do Python gera `uvicorn.exe` (e outros scripts instalados via `pip install`) como um executável pequeno com o caminho absoluto do `python.exe` da própria `.venv` gravado dentro dele, no momento da instalação — não é um caminho relativo recalculado a cada execução. Renomear a pasta que contém a `.venv` (ou movê-la) não atualiza esse caminho gravado; o `uvicorn.exe` continua tentando executar o `python.exe` no caminho antigo, que não existe mais, e falha antes de conseguir escrever qualquer coisa nos arquivos de log redirecionados — daí os logs vazios.
