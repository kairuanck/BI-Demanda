...
## 8. Adendo — Windows sem Git nem administrador

O mesmo usuário também não conseguiu instalar o Git (necessário para rodar um script `.sh` no Windows) — perfil sem permissão de administrador. Adicionei uma terceira opção, **`iniciar-sem-docker.ps1` / `parar-sem-docker.ps1`**, equivalente em PowerShell (já vem em qualquer Windows, não exige instalar nada além de Python/Node). `PRIMEIRO_USO.md` passou a explicar como instalar os dois sem administrador (Python via "Customize installation" sem "Install for all users"; Node.js via `.zip` portátil).

**Diferente das outras duas versões, esta não pôde ser validada numa máquina Windows real** — este ambiente não tem Windows disponível, só revisão cuidadosa de código. Mesmo assim, a revisão encontrou e corrigiu dois problemas antes de entregar: `Start-Process` sem `-WorkingDirectory` explícito faria o backend/frontend não encontrarem seus próprios arquivos (não herda o diretório atual de forma confiável no Windows); e uma camada preventiva de limpeza de processos-filho em `parar-sem-docker.ps1`, pelo mesmo motivo do bug real já encontrado na versão Bash (seção 7). Deixei claro para o usuário que, sem teste real em Windows, esse é o caminho com mais chance de precisar de um ajuste na prática — e pedi para ele reportar a mensagem exata de qualquer erro. Detalhes em `docs/DECISIONS.md`, seção 34.
